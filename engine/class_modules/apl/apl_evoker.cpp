...
void augmentation( player_t* p )
{
  ...
  default_->add_action( "variable,name=pool_for_id,if=talent.imminent_destruction,default=0,op=set,value=(variable.eons_remains<8&talent.breath_of_eons|cooldown.deep_breath.remains<8&!talent.breath_of_eons)&essence.deficit>=1&!buff.essence_burst.react" );
  ...
  default_->add_action( "breath_of_eons,if=talent.wingleader&(fight_style.dungeonroute|fight_style.dungeonslice)&target.time_to_die>=18&evoker.allied_cds_up>0" );
  default_->add_action( "breath_of_eons,if=talent.wingleader&(target.time_to_die>=18&(raid_event.adds.in>=25|!raid_event.adds.exists|raid_event.adds.remains>=18&raid_event.adds.count%1.5<active_enemies))&!variable.wingleader_force_timings&(time%%240<=190&time%%240>=3|fight_style.dungeonroute|fight_style.dungeonslice)|fight_remains<=30&!fight_style.dungeonroute" );
  default_->add_action( "breath_of_eons,if=((cooldown.ebon_might.remains<=4|buff.ebon_might_self.up)&target.time_to_die>15&raid_event.adds.in>15|fight_remains<30&!fight_style.dungeonroute)&cooldown.allied_virtual_cd_time.up|fight_remains<=15&(talent.imminent_destruction|talent.melt_armor)&!fight_style.dungeonroute" );
  default_->add_action( "breath_of_eons,if=fight_style.dungeonroute&(target.time_to_die>=18&(raid_event.adds.in>=25|!raid_event.adds.exists|raid_event.adds.remains>=18&raid_event.adds.count%1.5<active_enemies))" );
  ...
  default_->add_action( "time_skip,if=(((cooldown.fire_breath.remains>?20)+(cooldown.upheaval.remains>?20)))>=30&(cooldown.breath_of_eons.remains>=20&talent.breath_of_eons|!talent.breath_of_eons&cooldown.deep_breath.remains>=20)&(!buff.imminent_destruction.up|!action.eruption.usable)" );
